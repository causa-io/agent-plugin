...
- A column added to a table that already holds rows is `NOT NULL` with a `DEFAULT`, so the `ALTER` backfills them. This follows from the matching JSONSchema property being optional rather than nullable: optional is about events published before the property existed, not about the column. A property that is genuinely nullable maps to a nullable column instead, usually with no default, so it defaults to `NULL`.

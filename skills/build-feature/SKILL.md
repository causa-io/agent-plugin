...
Ask in **two waves**, using the inventory in `./design-questions.md`:
...
Run `review-design` **in a clean context**. Give it only the work directory path, the domain, and the base branch — never your reasoning, and never a summary of your decisions. A reviewer that inherits the designer's context defends the design instead of challenging it.

Run it wherever your context does not follow — a subagent, failing that a fresh session given only those paths, failing that the human running it themselves. Say which, so the review can be weighed accordingly.
...
Ask for approval to move to implementation. End by suggesting the context be compacted, or the session restarted, before implementation — the design, the contracts, and the behaviors are all on disk.
...
Run `review-implementation` **in a clean context**, with only the branch name, the domain, the work directory path, and the base branch. Isolate it as in step 6.
...
Invoke `improve-skills`. It reads the review dispositions, the deviations, and `feedback.md`, and writes up the skill changes that would have prevented what had to be corrected. Without it, everything learned here is lost at the end of the session.

## 14. Conclude

Tell the user that the feature has been designed, implemented, documented, and reviewed, that `skill-feedback.md` is waiting to be applied in the plugin repository if it proposes anything, and that the version bump and pull request are separate steps.
...
- `design-review.md` and `implementation-review.md` — written by the reviews.
...
5. Both reviews ran in a clean context that received paths only, never the reasoning behind the work, and the isolation mechanism used was stated.
...
- An assumption you recorded turns out to determine a decision you cannot reverse cheaply.
...
| `review-design` | review | `design-review.md` |
| `review-implementation` | review | `implementation-review.md` |
...
| `improve-skills` | action | `skill-feedback.md` |

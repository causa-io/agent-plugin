...
**Note for implementation:** The abstract classes define architectural contracts, not final implementations. The plan is a briefing, not a specification to conform to. Deviating from it while writing the code can be expected. Record the deviation and the reason rather than forcing the code to match.
...
6. Every method signature was checked against each of its call sites: it returns only what a caller uses, and every argument is available at the call site before the call is made.
7. The plan opens with the Decisions, Rejected, and Risks sections.
8. No contract, database schema, or `causa.yaml` file was modified.

...
The plan is a briefing, not a specification. Deviating from it is expected, problems that span several classes and modules rarely surface before the code exists. What matters is that the deviation is visible.
...
Also record, under a `## Direct tests` heading, any logic you covered with unit tests rather than through the contract, and why — a pure computation, a combinatorial case, retry logic, etc. One line each.

Behaviors the design already listed do not need an entry here: they were specified, not discovered. This section is for what only became apparent once the code existed.

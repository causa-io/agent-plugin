...
5. Every assumption in `requirements.md` was assessed, and those whose correction would change a contract are called out.
...
- Lifecycle: can every state be reached and left? Is any transition unreachable, or terminal by accident? Where a state depends on another entity, run the same check against that entity's lifecycle: if a change to it can remove the only exit from a state, the design must say what happens to an entity left in it.
...
- Does a table holding entities or projections with a `deletedAt` property declare a row deletion policy, so soft-deleted rows are eventually removed?
...
- Does every rule the requirements or a third-party's documentation define have a line — a computation, a threshold, a retry policy? These have no endpoint or event to make them visible, so they are the ones the list omits.

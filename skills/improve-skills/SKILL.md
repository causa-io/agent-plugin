...
**You do not edit any skill.** You produce one document, `skill-feedback.md`. The skills live in a different repository. Applying the changes is the job of an agent working in that repository, and that agent's only input is the document you write. Everything up to that point happens in the repository where the feature was built.

You do not re-litigate feedback that was rejected. A skill that grows by paraphrase loses the specificity that made it work, so every change you write up is the smallest edit that would have changed the outcome.

Proposing no change is a valid and frequent outcome. A skill edited on the strength of a single one-off is worse than one left alone.
...
- **The branch's commits**, but only for the code, and only when the branch was committed incrementally: a commit that rewrites or reverts an earlier one on the same branch marks something that was corrected. Git says *what* changed, never *why* — pair anything you find here with an entry from another source, or drop it.
...
- **Repository-specific** — a business rule, a domain value, a decision that binds this codebase but not others. Becomes an update to the repository's own memory, not to a skill shipped to everyone.
...
Write `skill-feedback.md` in the work directory, following the structure in "Output".

Write it for a reader who has none of your context: a different agent, in the plugin's repository, with no access to this session, this codebase, or this work directory. Every lesson must therefore carry its own evidence **quoted inline**, not referenced by a path they cannot open. A proposal that says "see `design.md`" is unusable.

For each proposed change, state precisely enough that the downstream agent does not have to re-derive it:
...
Repository-specific lessons are the exception: they belong to the repository you are already in. Propose them in the same document, and apply them there once the user approves.

</instructions>

<output>

A Markdown file named `skill-feedback.md`, in the work directory at `domains/<domain>/work/<feature-slug>/`.

Repository-specific lessons may additionally be applied in the repository being worked on, once the user approves them.
...
1. Every source was checked: `feedback.md`, both review documents, the deviations, the conversation, and the pull request if one exists.
...
6. Every proposed change names exactly one destination, the section within it, and the exact text to add or replace.
...
Skills degrade by accumulation as surely as by omission. Nothing downstream will second-guess what you write up, so restraint belongs here. Each run:

- Propose a change only where the lesson earns it. A lesson whose failure mode recurred, or whose consequence was severe, earns one; one drawn from a single harmless occurrence does not, and belongs in the proposal as observed but not acted on.
- Prefer strengthening a `<validation>` list to adding a guideline. Validation items are checked; prose is skimmed.
- Keep the proposed edit minimal: a bullet in an existing section, not a rewritten skill.

...
- Document schema files are named after the entity they project, in kebab-case: `<entity>.yaml`, e.g. `my-entity.yaml`.
- The schema title should follow the pattern `<EntityName>Document`, e.g. `MyEntityDocument`.
...
- **Root collections** (e.g., `/myEntities/{id}`) should be preferred for entities with a single unique ID.
- **Nested collections** (e.g., `/users/{userId}/myEntities/{myEntityId}`) should be used when the entity has a composite key. The path should reflect the composite key structure.
